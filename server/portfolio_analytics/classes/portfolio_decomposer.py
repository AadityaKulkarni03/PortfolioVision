...
class PortfolioDecomposer:
    ...
    def decompose_stocks(self):
        ...
        # Round allocation first
        grouped_stocks["allocation"] = grouped_stocks["allocation"].round(2)

        # Compute portfolio weights
        total_alloc = (grouped_stocks["allocation"].sum())
        grouped_stocks["port_weight_pct"] = (grouped_stocks["allocation"] / total_alloc *100).round(2)
    ...
    def decompose_sectors(self):
        ...
        sector_df["port_weight_pct"] = (sector_df["allocation"] / total_alloc *100).round(2)

        # ✅ Sort sectors by descending allocation
        sector_df = sector_df.sort_values(by="allocation", ascending=False).reset_index(drop=True)
