...
class MarketData:
    ...
    def _process_etf_data(self, parameter):
        ...
        for ticker in etf_tickers:
            ...
            if not data:
                # Instead of just printing, store placeholder or default
                if parameter == 'sector':
                    df = pd.DataFrame([{'sector': 'ETF'}])  # Or 'Unknown'
                elif parameter == 'holdings':
                    df = pd.DataFrame([{'symbol': ticker, 'holding': 0}])  # Or NaN
                else:
                    df = pd.DataFrame()
        ...
        if missing_data:
            print(f"{parameter} data not available for: {', '.join(missing_data)}")
